...
export default class Contract {
    ...
    initialize(callback) {
        ...
        this.web3.eth.getAccounts((error, accts) => {
            ...
            self.flightSuretyData.methods.authorizeCaller(this.appAddress).send({from: this.owner}, (error, result) => {
                if(error) {
                ...
                if (error) {
                    ...
                    isRegistered(payload.airlineAddress).call({ from: payload.sender}, (error, result) => {
                        if (error || result.toString() === 'false') {
                            payload.message = 'New airline needs at least 4 votes to be registered.';
